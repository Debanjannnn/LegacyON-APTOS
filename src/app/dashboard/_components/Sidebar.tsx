...
export function SidebarDemo({ children, onSectionChange }: { children: React.ReactNode, onSectionChange?: (section: string) => void }) {
  ...
  return (
    <div
      className={cn("flex w-screen h-screen flex-row overflow-hidden bg-black")}
    >
      <Sidebar open={open} setOpen={setOpen} animate={false}>
        <SidebarBody className="justify-between gap-10 bg-black border-r border-white/10 shadow-2xl min-h-screen px-4 py-8 relative z-10">
          ...
          <div className="mt-auto">
            ...
            <div className="relative">
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-[#df500f]/20 to-[#ff6b35]/20 rounded-2xl blur-xl opacity-50"></div>

              {/* Main wallet container */}
              <div className="relative bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-4 shadow-2xl hover:shadow-[0_0_30px_rgba(223,80,15,0.3)] transition-all duration-300 hover:border-white/20">
                {/* Header */}
...
export const Logo = () => {
  return (
    <a
      href="#"
      className="relative z-20 flex items-center space-x-3 py-4 px-4 mb-6 rounded-2xl bg-gradient-to-r from-[#df500f] to-[#ff6b35] shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 border border-white/10"
    >
      <div className="h-7 w-8 shrink-0 rounded-xl bg-white/20 backdrop-blur-sm border border-white/10" />
      <motion.span
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
